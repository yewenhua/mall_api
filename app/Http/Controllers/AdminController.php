...
class AdminController extends Controller
{
                    ...
                    if ($id) {
                        ...
                        if ($type && $res) {
                            ...
                            if ($type == 'operate') {
                                ...
                                $role_id = config('user.role_maintenance_id');
                            } elseif ($type == 'agent') {
    ...
    public function storeRole(\App\User $user){
        ...
        if($flag) {
            ...
            foreach ($roles as $role){
                if(strpos($role->name, config('user.role_finance_name')) !== false){
                    $user->type = 'finance';
                }
                elseif(strpos($role->name, config('user.role_operate_name')) !== false){
                    $user->type = 'operate';
                }
                elseif(strpos($role->name, config('user.role_maintenance_name')) !== false){
                    $user->type = 'maintenance';
                }
                elseif(strpos($role->name, config('user.role_agent_name')) !== false){
                    $user->type = 'agent';
                }
                elseif(strpos($role->name, config('user.role_straight_name')) !== false){
                    $user->type = 'straight';
                }
                else{
                    $user->type = '';
